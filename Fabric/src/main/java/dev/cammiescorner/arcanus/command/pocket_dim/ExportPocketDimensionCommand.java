package dev.cammiescorner.arcanus.command.pocket_dim;
...
import dev.cammiescorner.arcanus.component.level.PocketDimensionPlot;
...
public class ExportPocketDimensionCommand {
	...
	private static int export(CommandContext<CommandSourceStack> context, GameProfile targetProfile) {
		...
		PocketDimensionPlot plot = PocketDimensionComponent.get(server).getAssignedPlotSpace(targetProfile.id());

		if(plot == null) {
			context.getSource().sendFailure(Component.translatable(COMMAND_EXPORT_POCKET_POCKET_NOT_FOUND, targetProfile.name()));
			return 0;
		}

		Identifier structureId = Arcanus.id("pocket_dimensions/%s/pocket_dimensions_%s".formatted(targetProfile.id(), date));
		Arcanus.LOGGER.info("Saving pocket dimension for player {} ({}) as '{}'", targetProfile.name(), targetProfile.id(), structureId);
		...
		structure.setAuthor("%s_pocket_dimension_%s_%s".formatted(Arcanus.MOD_ID, targetProfile.id(), date));
		...
		context.getSource().sendSuccess(() -> Component.translatable(COMMAND_EXPORT_POCKET_SUCCESS, targetProfile.name()), true);
