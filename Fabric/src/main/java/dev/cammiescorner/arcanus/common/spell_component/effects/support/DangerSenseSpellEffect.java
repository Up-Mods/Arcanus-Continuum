...
public class DangerSenseSpellEffect extends SpellEffect {
	public DangerSenseSpellEffect() {
		super(
			...
			() -> ArcanusConfig.SupportEffects.DangerSenseEffectProperties.activatesOnce
		);
	}
	...
	public void effect(@Nullable LivingEntity caster, @Nullable Entity sourceEntity, Level level, HitResult target, List<SpellEffect> effects, ItemStack stack, double potency) {
		if(target.getType() == HitResult.Type.ENTITY) {
			...
			if(entityHit.getEntity() instanceof LivingEntity livingEntity)
				livingEntity.addEffect(new MobEffectInstance(ArcanusMobEffects.DANGER_SENSE.holder(), ArcanusConfig.SupportEffects.DangerSenseEffectProperties.baseEffectDuration, (int) Math.min(potency - 1, 0), true, false));
		}
	}
}
