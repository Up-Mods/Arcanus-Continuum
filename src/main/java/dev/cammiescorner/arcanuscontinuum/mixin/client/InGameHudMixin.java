...
public class InGameHudMixin {
	...
	private void arcanuscontinuum$moveCrosshair(GuiGraphics gui, int x, int y, int u, int v, int w, int h, Operation<Void> original) {
		MatrixStack matrices = gui.getMatrices();
		matrices.push();
		matrices.translate((scaledWidth - 15) / 2F, (scaledHeight - 15) / 2F, 0);
		original.call(matrices, 0, 0, u, v, w, h);
		matrices.pop();
	}
}
