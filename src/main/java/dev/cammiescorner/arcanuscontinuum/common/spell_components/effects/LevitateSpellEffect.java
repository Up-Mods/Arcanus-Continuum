...
public class LevitateSpellEffect extends SpellEffect {
	public LevitateSpellEffect(SpellType type, ParticleEffect particle, Weight weight, double manaCost, int coolDown, int minLevel) {
	...
	public void effect(@Nullable LivingEntity caster, World world, HitResult target, List<SpellEffect> effects, StaffItem staffItem, ItemStack stack) {
		if(target.getType() == HitResult.Type.ENTITY) {
			...
			if(entityHit.getEntity() instanceof LivingEntity livingEntity)
				livingEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.LEVITATION, 60 * (int) (effects.stream().filter(effect -> effect == ArcanusSpellComponents.LEVITATE).count()), 0, true, false));
		}
	}
}
