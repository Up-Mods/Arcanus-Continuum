	...
	public void effect(@Nullable LivingEntity caster, @Nullable Entity sourceEntity, World world, HitResult target, List<SpellEffect> effects, ItemStack stack, double potency) {
		if(target.getType() == HitResult.Type.ENTITY) {
			...
			if(entityHit.getEntity() instanceof LivingEntity livingEntity)
				livingEntity.addStatusEffect(new StatusEffectInstance(ArcanusStatusEffects.ANONYMITY.get(), ArcanusConfig.UtilityEffects.AnonymityEffectProperties.baseEffectDuration * (int) (effects.stream().filter(ArcanusSpellComponents.ANONYMITY::is).count() * potency), 0, true, false));
		}
	}
}
