...
public class PushSpellEffect extends SpellEffect {
	public PushSpellEffect(SpellType type, ParticleEffect particle, Weight weight, double manaCost, int coolDown, int minLevel) {
	...
	public void effect(@Nullable LivingEntity caster, World world, HitResult target, List<SpellEffect> effects, StaffItem staffItem, ItemStack stack) {
		if(target.getType() == HitResult.Type.ENTITY) {
			...
			if(caster != null) {
				entity.addVelocity(caster.getRotationVector().multiply(effects.stream().filter(effect -> effect == ArcanusSpellComponents.PUSH).count() * 0.2));
