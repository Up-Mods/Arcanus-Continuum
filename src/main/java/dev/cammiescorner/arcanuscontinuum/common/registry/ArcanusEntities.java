...
public class ArcanusEntities {
	...
	public static final RegistrySupplier<EntityType<AggressorbEntity>> AGGRESSORB = ENTITY_TYPES.register("aggressorb", () -> QuiltEntityTypeBuilder.create().entityFactory(AggressorbEntity::new).trackingTickInterval(60).disableSummon().setDimensions(EntityDimensions.fixed(0.6F, 0.6F)).build());
	public static final RegistrySupplier<EntityType<PocketDimensionPortalEntity>> PORTAL = ENTITY_TYPES.register("pocket_dimension_portal", () -> QuiltEntityTypeBuilder.create().entityFactory(PocketDimensionPortalEntity::new).disableSummon().setDimensions(EntityDimensions.fixed(1.5f, 0.1f)).build());
}
