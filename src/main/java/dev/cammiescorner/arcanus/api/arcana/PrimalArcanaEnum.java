...
public enum PrimalArcanaEnum implements Arcana, StringRepresentable {
	...
	public static final Codec<PrimalArcanaEnum> CODEC = StringRepresentable.fromEnum(PrimalArcanaEnum::values);
	public static final StreamCodec<FriendlyByteBuf, PrimalArcanaEnum> STREAM_CODEC = StreamCodec.ofMember((type, buf) -> buf.writeEnum(type), buf -> buf.readEnum(PrimalArcanaEnum.class));
	...
	PrimalArcanaEnum(RegistrySupplier<Attribute> arcanaAttribute, RegistrySupplier<Attribute> regenAttribute, ChatFormatting formatting, Color color, String name) {
	...
	public static PrimalArcanaEnum getByName(String serializedName) {
		var optional = Arrays.stream(PrimalArcanaEnum.values()).filter(arcanaType -> arcanaType.getSerializedName().equals(serializedName)).findFirst();
		return optional.orElse(PrimalArcanaEnum.IGNIS);
	}
}
